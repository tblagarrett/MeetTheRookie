...
// demonstration how to build a simple Dialogue View that
...
{
    [SerializeField] private float appearanceTime = 0.5f; // The amount of time that lines will take to appear.

    [SerializeField] private float disappearanceTime = 0.5f; // The amount of time that lines will take to disappear.

    
    [SerializeField] TMPro.TextMeshProUGUI text; // The text view to display the line of dialogue in.

    [SerializeField] RectTransform container; // The game object that should animate in and out.
    ...
    public void Start()
    {
        ...
        Scale = 0;
        playerInteraction = GameObject.FindGameObjectWithTag("Player").GetComponent<Player_Interaction>();
    }
    ...
    {   //to reference the char name --> dialogueLine.CharacterName
        IEnumerator PresentLine()
        {
            text.gameObject.SetActive(true);
            text.text = dialogueLine.TextWithoutCharacterName.Text;
            if(dialogueLine.CharacterName=="Player"){
                bubbleParent.transform.position=playerInteraction.IconMath();
            }
